// src/lib/answer-export-utils.ts
...
export function getAnswer(q: ExamQuestion): string {
  if (q.correct_answer && q.correct_answer.trim()) return q.correct_answer.trim()
  return parseAnswerFromLatex(q)
}

export function parseAnswerFromLatex(q: ExamQuestion): string {
...
export function parseMCAnswer(latex: string): string {
...
export function parseSAAnswer(latex: string): string {
  const m = latex.match(/\\shortans(?:\[[^\]]*\])?\{([^}]+)\}/)
  return m ? m[1].trim() : ''
}

export function parseTFAnswer(latex: string): string {
 ...
 */
export function generateTNMakerExcel(examSets: ExamQuestion[][], examCodes: string[]): Buffer {
...
// ─── YoungMix Excel ────────────────────────────────────────────────────────────
export function generateYoungMixExcel(examSets: ExamQuestion[][], examCodes: string[]): Buffer {
  const dataRows: CellValue[][] = []
...
// ─── SmartTest Excel ──────────────────────────────────────────────────────────
export function generateSmartTestExcel(examSets: ExamQuestion[][], examCodes: string[]): Buffer {
  const canonical = examSets[0] ?? []
  const mcQs   = canonical.filter(q => q.question_type === 'multiple_choice')
  const tfQs   = canonical.filter(q => q.question_type === 'true_false')
  const saQs   = canonical.filter(q => q.question_type === 'short_answer')
 ...
 */
export function generateAZOTAExcel(examSets: ExamQuestion[][], examCodes: string[]): Buffer {
  const canonical = examSets[0] ?? []
  const mcQs = canonical.filter(q => q.question_type === 'multiple_choice')
  const tfQs = canonical.filter(q => q.question_type === 'true_false')
  const saQs = canonical.filter(q => q.question_type === 'short_answer')
 ...
 */
export function generateOLMExcel(examSets: ExamQuestion[][], examCodes: string[]): Buffer {
  const canonical = examSets[0] ?? []
  const mcQs = canonical.filter(q => q.question_type === 'multiple_choice')
  const tfQs = canonical.filter(q => q.question_type === 'true_false')
  const saQs = canonical.filter(q => q.question_type === 'short_answer')
...
export function buildAnswerSheetTex(examCode: string, mcCount: number, tfCount: number, saCount: number): string {
  ...
  tex += `\t\\def\\madephieu{${madeDigits}}%mã đề\n`
