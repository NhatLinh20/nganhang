...
exports.VALID_CHAPTERS = exports.VALID_VARIANTS = void 0;
exports.validateCategoryCode = validateCategoryCode;
exports.parseCategoryCode = parseCategoryCode;
exports.extractComments = extractComments;
exports.findValidCategoryCode = findValidCategoryCode;
...
//   - Dạng: 0-9
// ═══════════════════════════════════════════════════
var CATEGORY_CODE_REGEX = /^([012])([DHC])(\d)([NHVC])(\d)-([\d])$/;
/**
...
exports.VALID_CHAPTERS = (function () {
    var result = { '0': {}, '1': {}, '2': {} };
    for (var _i = 0, _a = Object.keys(exports.VALID_VARIANTS); _i < _a.length; _i++) {
        var key = _a[_i];
        var grade = key[0];
        var subject = key[1];
        var chapter = key[2];
...
function validateCategoryCode(code) {
    ...
    var match = trimmed.match(CATEGORY_CODE_REGEX);
    if (!match) {
        return { valid: false, error: 'Không đúng định dạng ID 6 tham số' };
    }
    var gradeCode = match[1], subjectArea = match[2], chapterStr = match[3], lessonStr = match[5], variantStr = match[6];
    var subjectName = subjectArea === 'D' ? 'Đại số' : subjectArea === 'H' ? 'Hình học' : 'Chuyên đề';
    var gradeName = gradeCode === '0' ? '10' : gradeCode === '1' ? '11' : '12';
    // Kiểm tra lớp
    var validForGrade = exports.VALID_CHAPTERS[gradeCode];
    if (!validForGrade)
        return { valid: false, error: 'Lớp không hợp lệ' };
    // Kiểm tra phân môn
    var validChapters = validForGrade[subjectArea];
    if (!validChapters)
        return { valid: false, error: "Ph\u00E2n m\u00F4n ".concat(subjectName, " kh\u00F4ng t\u1ED3n t\u1EA1i \u1EDF l\u1EDBp ").concat(gradeName) };
    // Kiểm tra chương
    if (!validChapters.includes(chapterStr)) {
        return {
            valid: false,
            error: "Ch\u01B0\u01A1ng ".concat(chapterStr, " kh\u00F4ng thu\u1ED9c m\u00F4n ").concat(subjectName, " l\u1EDBp ").concat(gradeName)
        };
    }
    // Kiểm tra bài (lesson)
    var chapterKey = "".concat(gradeCode).concat(subjectArea).concat(chapterStr);
    var validLessons = exports.VALID_VARIANTS[chapterKey];
    if (!validLessons) {
        return { valid: false, error: "Kh\u00F4ng t\u00ECm th\u1EA5y d\u1EEF li\u1EC7u b\u00E0i cho ".concat(chapterKey) };
    }
    var validVariantsForLesson = validLessons[lessonStr];
    if (!validVariantsForLesson) {
        var availableLessons = Object.keys(validLessons).join(', ');
        return {
            valid: false,
            error: "B\u00E0i ".concat(lessonStr, " kh\u00F4ng t\u1ED3n t\u1EA1i trong Ch\u01B0\u01A1ng ").concat(chapterStr, " ").concat(subjectName, " l\u1EDBp ").concat(gradeName, " (c\u00E1c b\u00E0i h\u1EE3p l\u1EC7: ").concat(availableLessons, ")")
        };
    }
    // Kiểm tra dạng (variant)
    if (!validVariantsForLesson.includes(variantStr)) {
        var availableVariants = validVariantsForLesson.join(', ');
        return {
            valid: false,
            error: "D\u1EA1ng ".concat(variantStr, " kh\u00F4ng t\u1ED3n t\u1EA1i trong B\u00E0i ").concat(lessonStr, ", Ch\u01B0\u01A1ng ").concat(chapterStr, " ").concat(subjectName, " l\u1EDBp ").concat(gradeName, " (c\u00E1c d\u1EA1ng h\u1EE3p l\u1EC7: ").concat(availableVariants, ")")
        };
    }
    return { valid: true };
}
 ...
 */
function parseCategoryCode(code) {
    var trimmed = code.trim();
    var validation = validateCategoryCode(trimmed);
    if (!validation.valid)
        return null;
    var match = trimmed.match(CATEGORY_CODE_REGEX);
    var gradeCode = match[1], subjectArea = match[2], chapterStr = match[3], difficulty = match[4], lessonStr = match[5], variantStr = match[6];
 ...
 */
function extractComments(latexBlock) {
    var results = [];
 ...
 */
function findValidCategoryCode(comments) {
    for (var _i = 0, comments_1 = comments; _i < comments_1.length; _i++) {
        var comment = comments_1[_i];
        var info = parseCategoryCode(comment);
        if (info)
            return info;
    }
    return null;
 ...
 */
function detectImageType(latexBlock) {
