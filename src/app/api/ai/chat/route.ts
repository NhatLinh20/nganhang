...
- BẮT BUỘC dùng lệnh \\\\ để xuống dòng trong phần lời giải (\\loigiai) hoặc khi viết các gạch đầu dòng (-). Việc chỉ dùng phím Enter để xuống dòng sẽ khiến mã LaTeX không ngắt dòng khi hiển thị. VD đúng: - Điều kiện: $x > 0$.\\\\`
