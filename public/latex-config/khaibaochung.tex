\usepackage{amsmath,amssymb,mathrsfs,fancyhdr,enumerate,multirow,makecell,currfile,venndiagram,fontawesome,pifont}
\usepackage{mathpazo}
...
%					\node[rotate=90] at ($(current page.east)+(-1.26cm,0)$) {\fontsize{20pt}{15pt}\fontfamily{qag}\selectfont #1};
%					\ifnum\ktra=1
%					\else
%					{\node at (current page.center) {\includegraphics[width=0.5\paperwidth]{LogoVnMaths.png}};}
%					\fi
%			\end{tikzpicture}}
%			\else
%			{\begin{tikzpicture}[remember picture,overlay,scale=.5]
%					\node[rotate=90] at ($(current page.west)+(1.26cm,0)$) {\fontsize{20pt}{15pt}\fontfamily{qag}\selectfont#2};
%					\ifnum\ktra=1
%					\else
%					{\node at (current page.center) {\includegraphics[width=0.5\paperwidth]{LogoVnMaths.png}};}
%					\fi
...
%	\VnMathsLatex{ĐỀ THAM KHẢO GIỮA KỲ 2 - HỌC KỲ 2, TOÁN 12}{\faGroup~NHÓM VN - MATHS \& \LaTeX}{2}}
...
\newenvironment{name}[6]{
	\setcounter{page}{1}
	\noindent
	\begin{minipage}[]{0.4\textwidth}
		\centering
		{\large\color{red}\bfseries\MakeUppercase{#1}}\\
		{\bfseries\color{blue}{#2}}\\
		\textit{#3}
	\end{minipage}
	\begin{minipage}[]{0.6\textwidth}
		\color{black}	\centering
		{\large\bfseries\selectfont\MakeUppercase {#4}}\\
		{\bfseries\color{black}{#5}}\\
		\textit{#6}
	\end{minipage}
	\\[1mm]
...
\fancyfoot[LE,RO]{Trang \thepage \,--\, mã đề \made}
